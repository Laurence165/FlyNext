...
export default function HotelsList() {
  ...
  const handleDeleteHotel = (hotelId: string) => {
    // TODO: call an API to delete the hotel
    toast({
      title: "Hotel deleted",
          ...
          {mockHotels.map((hotel) => (
            <Card key={hotel.id} className="overflow-hidden">
              ...
              <CardContent className="p-4">
                ...
                <div className="mt-2">
                  <p className="text-sm">
                    <span className="font-medium">Rooms:</span> {hotel.totalRooms}
                  </p>
                  <p className="text-sm">
                    <span className="font-medium">Room Types:</span> {hotel.roomTypes.length}
                  </p>
                </div>
              </CardContent>
              <CardFooter className="p-4 pt-0 flex justify-between">
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/hotel-owner/hotels/${hotel.id}`}>
